        ...
        <section className="border-b border-rule" style={{ background: "#F5EFE6" }}>
          <div className="mx-auto max-w-[1600px] px-6 py-4 lg:px-8">
            <div className="flex flex-wrap items-center justify-between gap-4">
              ...
              <input type="text" placeholder={lang === "ca" ? "Cercar estàndard..." : "Buscar estándar..."} value={search} onChange={(e) => setSearch(e.target.value)} className="border px-3.5 py-2 text-sm" style={{ background: "white", borderColor: "#C9B89A", color: "#2C1810", width: "260px" }} />
            </div>
          </div>
        </section>
          ...
          <div className="mx-auto max-w-[1600px] px-6 lg:px-8">
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {filtered.map((s, i) => {
                ...
                return (
                  <div key={s.slug} className="flex cursor-pointer flex-col border transition-all hover:shadow-md hover:-translate-y-0.5" style={{ borderColor: "#C9B89A", background: "white" }} onClick={() => router.push(`/estandares-esg/${s.slug}`)}>
                    {/* Banda superior de color */}
                    <div className="h-1.5" style={{ background: cfg.borderColor }} />

                    {/* LOGO GRAN — block sencer */}
                    <div className="flex h-32 items-center justify-center border-b px-6" style={{ borderColor: "rgba(201,184,154,0.5)", background: "#FCFAF5" }}>
                      <Image
                        src={s.logo}
                        alt={`Logo ${s.name}`}
                        width={220}
                    ...
                    <div className="flex flex-1 flex-col gap-2.5 p-5">
                      ...
                      {/* Emissor */}
                      <p className="font-mono text-[10px] uppercase tracking-[0.12em]" style={{ color: "#8B7355" }}>{tr(s.issuerCa, s.issuerEs)}</p>

                      {/* Descripció */}
                      <p className="text-[12px] leading-relaxed" style={{ color: "#5C3A1E", display: "-webkit-box", WebkitLineClamp: 5, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{tr(s.descCa, s.descEs)}</p>
                      ...
                      <div className="mt-auto flex items-center justify-between border-t pt-3" style={{ borderColor: "rgba(201,184,154,0.5)" }}>
                        <span className="font-mono text-[10px] uppercase tracking-[0.14em]" style={{ color: "#8B7355" }}>
                          <strong style={{ color: "#B87333", fontWeight: 700, fontSize: "13px" }}>{s.count}</strong> {lang === "ca" ? "informes cross-ref" : "informes cross-ref"}
                        </span>
                        <span className="font-mono text-[10px] uppercase tracking-[0.14em] font-semibold" style={{ color: "#B87333", borderBottom: "1px solid #B87333", paddingBottom: "2px" }}>Ver →</span>
                      </div>
                    </div>
                  </div>
                );
