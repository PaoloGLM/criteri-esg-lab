...
export default function InformesPage() {
  ...
  const typeColors: Record<string, { bg: string; text: string }> = {
    regulatory: { bg: "rgba(92,58,30,0.12)", text: "#5C3A1E" },
    framework: { bg: "rgba(184,115,51,0.12)", text: "#B87333" },
    rating: { bg: "rgba(184,115,51,0.12)", text: "#B87333" },
    official: { bg: "rgba(92,58,30,0.12)", text: "#5C3A1E" },
      ...
      <main className="flex-1">
        ...
        <section className="border-b border-rule" style={{ background: "#F5EFE6" }}>
          <div className="mx-auto max-w-7xl px-6 py-16 lg:px-8 lg:py-20">
            ...
            <p className="max-w-2xl font-serif text-lg italic" style={{ color: "#5C3A1E" }}>
              {lang === "ca"
        ...
        <section className="py-10" style={{ background: "#F5EFE6" }}>
          <div className="mx-auto max-w-7xl px-6 lg:px-8">
            ...
            {featuredReport && (
              ...
              >
                <div className="flex flex-col gap-4 p-8">
                  ...
                  <p className="font-mono text-[10px] uppercase tracking-[0.14em]" style={{ color: "#8B7355" }}>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {featuredReport.certifications.map((cert) => (
                      <span key={cert} className="font-mono text-[9px] uppercase tracking-[0.12em] font-semibold px-2 py-1 border" style={{ borderColor: "#B87333", color: "#5C3A1E", background: "white" }}>{cert}</span>
                    ))}
                  </div>
                  <button className="self-start font-sans text-[13px] font-semibold" style={{ color: "#B87333", borderBottom: "1px solid #B87333", paddingBottom: "4px" }}>
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between">
                      ...
                      <span className="font-mono text-[9px]" style={{ color: "#8B7355" }}>{formatDate(report.date, lang)}</span>
                    </div>
                    <h3 className="font-serif text-lg font-medium leading-tight text-primary" style={{ letterSpacing: "-0.008em" }}>{report.title}</h3>
                    <p className="font-mono text-[9.5px] uppercase tracking-[0.14em]" style={{ color: "#8B7355" }}>
                    ...
                    <div className="mt-auto flex flex-wrap gap-1">
                      {report.certifications.slice(0, 4).map((cert) => (
                        <span key={cert} className="font-mono text-[8px] uppercase tracking-[0.1em] font-semibold px-1.5 py-0.5" style={{ background: "rgba(184,115,51,0.08)", color: "#5C3A1E" }}>{cert}</span>
                      ))}
                    </div>
                  </article>
                );
            ...
            {visibleCount < filteredReports.length && (
              <div className="mt-8 flex flex-col items-center gap-3 border-t pt-8" style={{ borderColor: "#C9B89A" }}>
                <button
                  onClick={() => setVisibleCount((c) => c + 6)}
                  className="px-9 py-3.5 text-sm font-semibold border"
                  style={{ background: "transparent", color: "#2C1810", borderColor: "#2C1810" }}
                >
