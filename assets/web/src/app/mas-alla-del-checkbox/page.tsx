      ...
      <main className="flex-1">
        ...
        <section style={{ background: "#2C1810", color: "#F5EFE6" }}>
          <div className="mx-auto max-w-4xl px-6 py-24 text-center lg:px-8 lg:py-28">
            <p className="mb-5 font-mono text-[11px] uppercase tracking-[0.22em] font-semibold" style={{ color: "#D9A574" }}>{tr("Sèrie editorial · Cada quinzena", "Serie editorial · Cada quincena")}</p>
            <h1 className="mb-6 font-serif text-6xl font-normal leading-tight" style={{ color: "#F5EFE6", letterSpacing: "-0.025em" }}>{tr("Més enllà del ", "Más allá del ")}<em className="italic" style={{ color: "#D9A574", fontWeight: 500 }}>Checkbox</em></h1>
        ...
        <section className="border-b border-rule" style={{ background: "#F5EFE6" }}>
          <div className="mx-auto max-w-4xl px-6 py-16">
            <div className="flex flex-col">
              {preguntas.map((p, i) => (
                <button key={p.num} onClick={() => router.push(`/mas-alla-del-checkbox/${p.num}`)} className="grid grid-cols-[60px_1fr_auto] gap-6 border-b py-8 text-left transition-colors hover:bg-[rgba(184,115,51,0.04)]" style={{ borderColor: "#C9B89A" }}>
                  <span className="font-serif text-4xl font-light" style={{ color: "#B87333", letterSpacing: "-0.03em" }}>{String(i + 1).padStart(2, "0")}</span>
                  <div className="flex flex-col gap-2">
                    <div className="flex gap-4 items-baseline font-mono text-[10px] uppercase tracking-[0.16px]" style={{ color: "#8B7355" }}>
                      <span style={{ color: "#8A5526", fontWeight: 600 }}>{p.date}</span>
                      <span className="px-2 py-0.5" style={{ background: "rgba(184,115,51,0.1)", color: "#8A5526", fontWeight: 600 }}>{p.type}</span>
                    </div>
                    ...
                    <p className="text-[13px] leading-relaxed" style={{ color: "#5C3A1E" }}>{tr(p.previewCa, p.previewEs)}</p>
                  </div>
                  <div className="flex flex-col items-end gap-2 pt-2">
                    <span className="font-mono text-[10px] uppercase tracking-[0.14px] font-semibold" style={{ color: "#B87333" }}>{tr("Llegir →", "Leer →")}</span>
                    <span className="font-mono text-[9px] uppercase tracking-[0.12px] font-semibold px-2 py-0.5" style={{ background: "#B87333", color: "white" }}>{tr("Dinàmica Premium", "Dinámica Premium")}</span>
                  </div>
                </button>
              ))}
            </div>
              ...
              <div className="flex flex-col gap-4">
                {[
                  ...
                  <div key={item.n} className="grid grid-cols-[32px_1fr] gap-4 border-b py-4" style={{ borderColor: "rgba(217,165,116,0.2)" }}>
                    <span className="font-serif text-2xl font-medium" style={{ color: "#D9A574" }}>{item.n}</span>
                    <p className="text-sm leading-relaxed" style={{ color: "#F5EFE6" }}><strong style={{ color: "#D9A574", fontWeight: 600 }}>{item.t}</strong> — {item.d}</p>
                  </div>
                ))}
              </div>
            </div>
