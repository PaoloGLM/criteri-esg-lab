...
def call_glm_flash_redactor(title: str, institution: str, destilat: str, aportacions: str, lang: str = "ca") -> str:
    ...
    system_prompt = (
        ...
        "BLOC 0 — SEMÀFOR METODOLÒGIC (format EXACTE obligatori):\n"
        "Taula Markdown amb EXACTAMENT aquests 5 criteris oficials, en aquest ordre:\n"
        "1. Slot 1 (el que sigui més adequat al tema): «Cobertura Scope 3» si l'informe tracta\n"
        "   emissions/cadena de valor; «Materialitat de l'impacte principal» en la resta.\n"
        "2. «Termes temporals»\n"
        "3. «Fonts independents»\n"
        "4. «Granularitat»\n"
        "5. «Verificació externa»\n\n"
        "Format:\n"
        "## Bloc 0 — Semàfor Metodològic\n\n"
        "**Nota global: X** — <una frase de síntesi>.\n\n"
        "| Indicador | Estat | Lectura |\n"
        "|---|---|---|\n"
        "| <criteri> | 🟢🟡 o 🔴 | <lectura concisa amb cita de pàgina> |\n\n"
        "- La Nota global es calcula NOMÉS amb el nombre de verds: 5V=A «Rigorós»;\n"
        "  3-4V=B «Acceptable amb matisos»; 1-2V=C «Feble metodològicament»; 0V=D «Materialitat qüestionable».\n"
        "- Cada Lectura acaba amb la cita de pàgina del document ORIGINAL: «(p. 12)» o «(p. 4, 17)».\n"
        "- MAI inventar un número de pàgina: si no el saps, escriu la lectura sense cita.\n"
        "- Els estats han de reflectir EVIDÈNCIA del document, no opinions.\n\n"
